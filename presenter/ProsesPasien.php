...
class ProsesPasien implements KontrakPresenter
{
	...
	function prosesById($pasienId) {
		try {
			//mengambil data di tabel pasien
			$this->tabelpasien->open();
			$this->tabelpasien->getPasienById($pasienId);
			while ($row = $this->tabelpasien->getResult()) {
				//ambil hasil query
				$pasien = new Pasien(); //instansiasi objek pasien untuk setiap data pasien
				$pasien->setId($row['id']); //mengisi id
	...
	function create($data) {
		try {
			//mengambil data di tabel pasien
			$this->tabelpasien->open();
			$result = $this->tabelpasien->create($data);
			//tutup koneksi
			$this->tabelpasien->close();
		} catch (Exception $e) {
			//memproses error
			$result =  "wiw error part 2" . $e->getMessage();
		}
		return $result;
	}

	function update($data, $pasien_id) {
		try {
			//mengambil data di tabel pasien
			$this->tabelpasien->open();
			$result = $this->tabelpasien->update($data, $pasien_id);
			//tutup koneksi
			$this->tabelpasien->close();
		} catch (Exception $e) {
			//memproses error
	...
	function delete($pasien_id) {
		try {
			//mengambil data di tabel pasien
			$this->tabelpasien->open();
			$result = $this->tabelpasien->delete($pasien_id);
			//tutup koneksi
			$this->tabelpasien->close();
		} catch (Exception $e) {
			//memproses error
